...
class veh_DataProcesser:
    ...
    def get_obs_pred(self):
        ...
        for pedIndex in range(len(self.traj_data)):
            for k in range(len(self.traj_data[pedIndex])):
                if len(self.traj_data[pedIndex])-k >= self.obs_length + self.pred_length:
                    obs_pedIndex = []
                    pred_pedIndex = []
                    count += 1
                    for i in range(self.obs_length):
                        obs_pedIndex.append(self.traj_data[pedIndex][i+k])
                    for j in range(self.pred_length):
                        pred_pedIndex.append(self.traj_data[pedIndex][j+k + self.obs_length])
    ...
    def get_traj_input(self):
        traj_input = []
        for pedIndex in range(len(self.obs_veh)):
            person_pedIndex = []
            for i in range(self.obs_length):
                person_pedIndex.append([self.obs_veh[pedIndex][i][-2], self.obs_veh[pedIndex][i][-1]])
            person_pedIndex = np.reshape(person_pedIndex, [self.obs_length, 2])

            traj_input.append(person_pedIndex)

        traj_input = np.reshape(traj_input, [len(self.obs_veh), self.obs_length, 2])

        return traj_input

    def get_expected_output(self):
        expected_ouput = []
        for pedIndex in range(len(self.pred_veh)):
            person_pedIndex = []
            for i in range(self.pred_length):
                person_pedIndex.append([self.pred_veh[pedIndex][i][-2], self.pred_veh[pedIndex][i][-1]])
            person_pedIndex = np.reshape(person_pedIndex, [self.pred_length, 2])

            expected_ouput.append(person_pedIndex)

        expected_ouput = np.reshape(expected_ouput, [len(self.pred_veh), self.pred_length, 2])
    ...
    def grid_map(self, neighborhood_size, dimensions, grid_size):
        ...
        for pedIndex in range(len(self.obs_veh)):
            group_pedIndex = []
            for i in range(self.obs_length):
                o_map_pedIndex = get_distance_map(self.obs_veh[pedIndex][i][1], self.obs_veh[pedIndex][i][0], dimensions,
